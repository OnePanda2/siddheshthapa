...
const tmp = (require('../tools/scratch.js').root() + '/aud-' + process.pid).replace(/\\/g, '/');
fs.mkdirSync(tmp, { recursive: true });
const page = tmp + '/a.html';
const probe = `<script>setTimeout(function(){
var r={};
try{
  var M=window.__v02; M.settle(90);
  var S=M.audit();
  r=S;
...
const cmd = '"C:/Program Files/Google/Chrome/Application/chrome.exe" --headless=new --hide-scrollbars' +
  ...
  ' --dump-dom "file:///' + page.replace(/\\/g,'/') + '#lite"';
...
console.log(JSON.stringify(JSON.parse(m[1].replace(/&quot;/g,'"').replace(/&amp;/g,'&')), null, 1));
