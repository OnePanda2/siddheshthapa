...
var V02_OVERLAY={
  ...
  /* nothing is renamed. The one entry that lived here renamed BUILDING to
     MESSAGES FOR THE ELITE, and that topic is gone rather than renamed. */
  relabel:[],
  ...
  /* nothing is pinned to the foot of the menu today. The mechanism stays: it
     is where a topic goes when it addresses a narrower reader than the rest. */
  menuLast:[],
  ...
  reline:[
    ...
    /* Two works the graph described as undocumented, because at the time they
       were. Both descriptions are now supplied by Siddhesh, so the lines that
       said nothing could be quoted are no longer true. The sheets stay
       reserved: a description is not a procedure. */
    { id:'p-statelab',
      to:"We're trying to build a random-number generator that's faster than the one securing most of the internet — without needing any special chip to run it — and after months of testing, we've found one candidate that's 3% faster and now have the first real evidence it might be genuinely stronger per unit of work, but 3% isn't enough to make anyone switch, so the hunt continues.",
      why:'replaces "Listed because it exists; what it does is not yet written down anywhere I can quote." It is written down now.' },
    { id:'p-automation',
  ...
  hideMIGs:[
    ...
    /* IT WAS BUILDING, THEN IT WAS MESSAGES FOR THE ELITE, AND NOW IT IS
       NEITHER. The second name was for writing that was going to sit behind a
       paywall, and that idea was dropped — deliberately, because material that
       could cost you your career is exposed the moment it is stored anywhere
       you do not control, whatever the access rules say.

       So the topic goes rather than being renamed a third time. Its five
       objects stay in the graph exactly as my-works's do: same ids, same
       relationships, no position, drawn nowhere and listed nowhere. Nothing is
       cut out of preview.html, which is locked and stays that way, and putting
       the region back is one line if it is ever wanted. */
    { id:'building',
      why:'the paywall it was renamed for was dropped, and an empty topic held open for writing that will not be published is a door onto nothing. Its objects survive in the graph; only the room is gone.' }
  ],
  /* THE COLLISION IS GONE, SO THE WORKAROUND IS GONE WITH IT.

     A Minor IG called PSYCHOLOGY has always lived inside HUMAN BEHAVIOUR. When
     PSYCHOLOGY was promoted to a topic of its own it needed the id, so the
     concept was re-keyed to psychology-behaviour to get out of the way — a
     rename that existed for one reason and no other.

     The promotion is undone: that world is BOOKS now, and nothing claims the
     id psychology except the concept that always held it. Keeping the rename
     would leave a workaround standing over a problem that no longer exists,
     and the next person to read it would have to work out that it protects
     nothing. The concept goes back to being called what it is. */
  renameIds:[],
  addMIGs:[
    /* BOOKS. This slot was added as PSYCHOLOGY and never written into, and
       PSYCHOLOGY belongs where it already was — a concept inside HUMAN
       BEHAVIOUR — rather than as a topic beside it. The world it was given,
       Kepler-62, stays exactly as it is: three planets close together, a gap
       of 3.56x where every other step is under 1.7, then two far out. An inner
       cluster and a distant pair, which is a reasonable shape for a shelf.

       Created EMPTY, and honestly so: nothing has been filed here, and no
       book was invented to make the topic look inhabited. */
    { id:'books', label:'BOOKS',
      gloss:'Not yet written. The topic exists; what has been read has not been filed here.',
      empty:true,
      why:'the slot that was added as PSYCHOLOGY, which belongs inside HUMAN BEHAVIOUR as the concept it already is. The world is unchanged; only what it is a world OF has changed.',
      conflict:null },
    /* THINGS I WAS WRONG ABOUT. A new topic, and the one system left
       unassigned was reserved for exactly this case: Proxima Cen's own record
       in the archive says "its emptiness is the point; the natural candidate
       for a MIG with almost no content". Two confirmed planets, both very
       close in, around the nearest star to the Sun.

       SOCIETY is untouched — it keeps Kepler-11 and all ten of its objects.
       This is a new topic beside it, not a renaming of it. */
    { id:'wrong', label:'THINGS I WAS WRONG ABOUT',
      gloss:'Positions I held and no longer hold. Kept rather than quietly deleted, because what changed a mind is worth more than what it now believes.',
      empty:true,
      why:'a topic of its own, created empty. Nothing is filed here yet and nothing was invented to fill it.',
      conflict:null },
    /* ART, with contents. It was a borrowed label until now: the MY WORKS
       entry was renamed to it while the works waited for a door of their own.
       They have one, so the borrowing ends and the region becomes itself.

       Everything below is extracted from Master Context §14, AESTHETIC
       SENSIBILITY, which the P4.7 corpus never indexed. Nothing is invented —
       the concepts are the list of what he says he cares about, and the
       beliefs are his sentences. */
    { id:'art', label:'ART',
      gloss:'Most of the thinking here is analytical. This is the other half — how a thing feels, not merely whether it functions.',
      empty:false,
      why:'promoted from a borrowed label to a region with its own contents, now that MY WORKS has its own door and no longer needs to hide behind this name.',
      src:'Master Context §14' }
  ],

  /* ART's contents. Concepts are the list §14 gives verbatim; beliefs are its
     sentences, quoted rather than paraphrased. Each carries its source. */
  addMinors:[
    { id:'visual-identity', label:'VISUAL IDENTITY', mig:'art', crosses:['business'], state:'formed' },
    { id:'interfaces',      label:'INTERFACES',      mig:'art', crosses:['technology'], state:'formed' },
    { id:'photography',     label:'PHOTOGRAPHY',     mig:'art', crosses:['observation'], state:'seed' },
    { id:'atmosphere',      label:'ATMOSPHERE',      mig:'art', crosses:['movies','music'], state:'formed' },
    { id:'presentation',    label:'PRESENTATION',    mig:'art', crosses:['business'], state:'formed' }
  ],
  addWritings:[
    { id:'b-feels', t:'belief', label:'HOW SOMETHING FEELS, NOT MERELY WHETHER IT FUNCTIONS',
      mig:'art', crosses:['building','technology'], state:'formed', register:'belief',
      src:'Master Context §14',
      line:'Analytical for most of the working day, and aesthetically sensitive underneath it. The test a thing has to pass is not only that it works.' },
    { id:'b-both-sides', t:'belief', label:'DO NOT ERASE EITHER SIDE',
      mig:'art', crosses:['philosophy','my-works'], state:'formed', register:'belief',
      src:'Master Context §14',
      line:'Two modes, and he operates between them: systems, logic, automation and strategy on one side; philosophy, aesthetics, meaning, nostalgia and contradiction on the other. A good representation of him should not erase either.' },
    { id:'t-substance', t:'thought', label:'SUBSTANCE AND EXPRESSION',
      mig:'art', crosses:['business'], state:'formed', register:'observation',
      src:'Master Context §14',
  ...
  addEdges:[
    ...
    ['b-feels','m-design','runs inside','The design loop is where the feel of a thing gets decided, iteration by iteration.'],
    ['b-both-sides','p-website','shapes','The site is the representation, and it is built so that neither half is left out.'],
    ['t-substance','b-package','agrees with','Take the parts you did not choose, and take the way they look as well as the way they work.'],
    ['atmosphere','photography','collected as','Both are attention paid to how a moment sits rather than to what it contains.'],
    ['presentation','entrepreneurship','sells through','Three years of positioning: the work is not the whole of what gets bought.']
  ]
};
/* EVERY REGION EVER DECLARED BEYOND THE LOCKED CORPUS, in the order it was
   declared, RETIRED ONES INCLUDED. This is the ledger the world claim counts
   against, and it is why retiring a topic cannot move another topic's sky:
   an entry is appended and never removed, so an index once handed out stays
   handed out. Same rule as a retired writing, which is blanked in place rather
   than spliced away because position in this mind is an index. */
var DECLARED_REGIONS=[];
(function(){
  /* ── REGIONS FROM THE STORE ───────────────────────────────────────────
     A topic used to be the one thing on this page that could not be added
     without editing src/. It is declared like anything else now: the editor
     writes it into data/notes.json and the build folds it in HERE, before the
     overlay is applied, so a store region goes through exactly the same
     addOnce guards, the same hide, the same everything as one written by hand.
     No second path, and therefore no second set of rules to keep in step. */
  (V02_NOTES.regions||[]).forEach(function(r){
    V02_OVERLAY.addMIGs.push({ id:r.id, label:r.label, gloss:r.line,
      empty:true, fromStore:true, added:r.added||null,
      why:'declared in the live note store rather than by hand — the topic was added from the editor.' });
  });
  /* AND RETIRING ONE IS A LINE ADDED, NOT A LINE REMOVED. It joins hideMIGs,
     which is the mechanism BUILDING and MY WORKS already go through: the room
     closes, its objects keep their ids and their books, and the relationships
     that pointed into it are swept rather than left drawing doors onto
     nothing. Its entry stays in the ledger above, so its world stays its own
     and nothing else shifts to take it. */
  (V02_NOTES.retiredRegions||[]).forEach(function(r){
    V02_OVERLAY.hideMIGs.push({ id:r.id, at:r.at||null, fromStore:true,
      why:r.why||'retired from the editor.' });
  });
  /* THE LOCKED CORPUS IS NOT IN THE QUEUE, and leaving it in was a real
     mistake for two builds: BUILDING and MY WORKS are corpus regions with no
     system, so they silently took the first two worlds out of the pool and
     held them without ever drawing a ring. They are not claimants. The corpus
     gave every region it means to show a sky of its own; the two it did not
     are rooms rather than topics, and both are hidden. So the queue is exactly
     the regions declared BEYOND the corpus, in declaration order. */
...
(function(){
  ...
  V02_OVERLAY.addMIGs.forEach(function(a){
    ...
    /* THE SENTENCE GOES IN THE FIELD THE PAGE ACTUALLY READS.
       This wrote `gloss`, and nothing anywhere reads `gloss` off a region —
       the corpus calls it `line` and so does every renderer, every form and
       applyEdits. So BOOKS, THINGS I WAS WRONG ABOUT and ART have each been
       standing on the live site under a name with no sentence beneath it,
       while the twelve corpus regions all had one. The overlay keeps its own
       word for the field, because that is what its three entries are written
       with; the push is what had to learn the graph's. */
    MIGS.push({ id:a.id, label:a.label, line:a.gloss, gloss:a.gloss,
                v02Added:true, v02Empty:!!a.empty });
  });
  /* CONTENTS FOR AN ADDED REGION. Same guard as the MIGs: never twice, and
     never over the top of something the locked corpus already declares. */
  function addOnce(list, rows){
    (rows||[]).forEach(function(r){
      for(var i=0;i<list.length;i++) if(list[i].id===r.id) return;
      list.push(r);
    });
  }
  /* A RELATIONSHIP INTO A HIDDEN ROOM IS A DOOR ONTO NOTHING.

     Hiding a region leaves its objects in the graph, which is right — they
     keep their ids and their history. But a relationship from a VISIBLE topic
     to one of them still renders a row, and that row goes nowhere: TECHNOLOGY
     showed "TOOLS · in messages for the elite", pointing at an object that
     exists nowhere a visitor can reach.

     MY WORKS is the exception and the reason this was never a problem before:
     its objects have somewhere to go, so a row pointing at one opens its sheet
     in the manual. A hidden region with no manual behind it has no such
     destination, so the relationships into it go with the room.

     Both directions, because an edge is a claim with a direction and a claim
     whose other end cannot be reached is not a claim anyone can follow. */
  /* THE SWEEP ITSELF RUNS AT THE END, once every source of edges has had its
     say — see below, after the live notes are merged. Placed here it ran
     before V02_OVERLAY.addEdges and missed one: b-feels, a belief in ART,
     pointing at THE DESIGN LOOP inside the room being closed. Five of the six
     were swept and the sixth was declared twenty lines later. */
  ...
  /* EDITED WRITINGS. preview.html is the locked source of truth and is never
     written to, so changing an original means DECLARING an override here - the
     same principle V02_OVERLAY already works by, and the reason relabel and
     reline exist. The original text stays in the corpus, recoverable, and what
     the page renders is the corpus with a stated correction laid over it.

     Applied before retirement, so a writing that was edited and later deleted
     stays blank, and before claims, so a claimed vacancy is filled by the new
     writing rather than by an override of the one it replaced. */
  var EDITS=V02_NOTES.edits||{};
  function applyEdits(list){
    list.forEach(function(n){
      var e=EDITS[n.id]; if(!e) return;
      Object.keys(e).forEach(function(k){
        /* an override may not move a writing to another region or change what
           kind of object it is: both decide where it sits and what draws it,
           and the graph is built from them before this could be honoured */
        if(k==="id"||k==="mig"||k==="t") return;
        n[k]=e[k];
      });
      n.edited=true;
    });
  ...
  applyEdits(THOUGHTS);
  applyEdits(MINORS);
  /* AND THE TOPICS THEMSELVES. A topic's description is the sentence a reader
     meets on arriving in it, and it was the one piece of prose on this page
     that could only be changed by editing the corpus. It is an override like
     any other now. */
  applyEdits(MIGS);
  ...
  /* A CLAIM FILLS A VACANCY OF ITS OWN KIND, and only its own kind. Concepts
     and writings are placed by different rules - in a planetary world the
     concepts take the orbits and the writings hang off them - so a writing
     dropped into a concept's orbit would be the wrong sort of body in it.
     Each list is therefore searched for its own claims and no others. */
  function claimInto(list, rows){
    var claims={}, fresh=[];
    (rows||[]).forEach(function(nn){
      if(nn.takes) claims[nn.takes]=nn; else fresh.push(nn);
    });
    addOnce(list, fresh);
    Object.keys(claims).forEach(function(oldId){
      for(var i=0;i<list.length;i++){
        if(list[i].id===oldId && list[i].vacant){ list[i]=claims[oldId]; return; }
      }
    });
  }
  ...
  /* A RELATIONSHIP INTO A HIDDEN ROOM IS A DOOR ONTO NOTHING.
     ...
     Hiding a region leaves its objects in the graph, which is right — they keep
     their ids and their history. But a relationship from a VISIBLE topic to one
     of them still renders a row, and that row goes nowhere: TECHNOLOGY showed
     "TOOLS · in messages for the elite", pointing at an object that exists
     nowhere a visitor can reach.

     MY WORKS is the exception and the reason this never came up before: its
     objects have somewhere to go, so a row pointing at one opens its sheet in
     the manual. A room with nothing behind it has no such destination, so the
     relationships into it go with it.

     LAST, DELIBERATELY. Edges arrive from three places — the corpus, the
     overlay, and the live store — and this has to see all three. Run earlier it
     swept the corpus's five and left the overlay's sixth standing, which is the
     one failure mode it exists to prevent. Both directions, because a claim
     whose other end cannot be reached is not a claim anyone can follow. */
  (function(){
    var vanish={};
    (V02_OVERLAY.hideMIGs||[]).forEach(function(h){
      if(h.id==='my-works') return;           // its objects have the manual
      MINORS.concat(THOUGHTS).forEach(function(o){ if(o.mig===h.id) vanish[o.id]=1; });
    });
    if(!Object.keys(vanish).length) return;
    var cut=0;
    for(var vi=EDGES.length-1;vi>=0;vi--)
      if(vanish[EDGES[vi][0]] || vanish[EDGES[vi][1]]){ EDGES.splice(vi,1); cut++; }
    V02_OVERLAY._edgesDroppedWithHiddenRooms=cut;
  })();

  /* ── A RELATIONSHIP RETIRED ON PURPOSE ────────────────────────────────
     A writing is listed under a concept because it is CONNECTED to it — the
     concept page is the adjacency, nothing more — so a writing that reaches
     into six concepts appears under all six, and one of those six may simply
     be wrong. There was no way to say so: relationships could be added from
     the editor and never taken away.

     They cannot be spliced out either. Most of them live in preview.html,
     which is the locked senior document, and the rest arrive through the
     overlay — so this follows the rule the retired writings already follow: a
     deletion is A LINE ADDED, not a line removed. The claim stays in the
     corpus, recoverable and legible, and the store declares that it no longer
     holds.

     MATCHED UNORDERED, because a relationship is one claim however it is
     written down, and the gate already refuses a pair that appears in both
     directions. Retiring a->b therefore cannot leave b->a standing.

     LAST, after everything that adds an edge — the corpus, the overlay, the
     live notes and the hidden-room sweep — for the same reason the sweep is
     last: a retirement that ran before an addition would be undone by it. */
  (function(){
    var gone={};
    (V02_NOTES.retiredEdges||[]).forEach(function(r){
      if(!r || !r.a || !r.b) return;
      gone[r.a+' '+r.b]=1; gone[r.b+' '+r.a]=1;
    });
    if(!Object.keys(gone).length) return;
    var cut=0;
    for(var ri=EDGES.length-1;ri>=0;ri--)
      if(gone[EDGES[ri][0]+' '+EDGES[ri][1]]){ EDGES.splice(ri,1); cut++; }
    V02_NOTES._edgesRetired=cut;
  })();
})();
...
var MIG_SYSTEM={ 'philosophy':'TRAPPIST-1', 'love':'Kepler-16', 'movies':'HR 8799',
                 ...
                 'food':'HD 219134',
                 'music':'Kepler-80', 'books':'Kepler-62', 'art':'HD 40307',
                 /* HD 10180, which BUILDING had and no longer needs. Six
                    measured orbits spreading from 0.064 to 3.4 AU — a real
                    system with room in it, where Proxima Cen's two planets
                    huddled inside 0.05 AU and read as a star with a smudge. */
                 'wrong':'HD 10180' };
...
/* LIFE is Kepler-33, five bodies evenly graded outward across 3.7x. Its gaps
   ...
   of Philosophy. */
/* TECHNOLOGY is GJ 876, and it is the widest span in the set at 16.1x: one
   very close inner body, then three locked together far outside it. A smaller
   base radius than the others because that span multiplies it — 9 puts the
   outer orbit at 145 rather than 209. */
/* BUSINESS is 55 Cnc, and its span is by far the widest here: 363x from an
...
var WORLD_SCALE={ 'philosophy':13, 'love':52, 'movies':18, 'life':15,
                  ...
                  'wrong':11, 'learning':20, 'behaviour':17,
                  'food':13 };
...
var FULL_SYSTEM={ 'music':1, 'books':1, 'wrong':1 };
/* where an unoccupied planet sits, per world, so the renderer can draw it */
var VACANT_PLANETS={};
/* the same planets as one flat list, in the order the point cloud writes them —
   render-only bodies, like the companion star and the background sky */
var VACANT_BODIES=[];
...
function orbitalSlots(tpl,R0,migId){
  ...
  var out=a.map(function(v){ return R0*(v/inner); });
  /* MORE CONCEPTS THAN MEASURED ORBITS.

     55 Cnc is reported as a seven-planet system and the archive returns
     semi-major axes for only five of them — the other two have no value in
     pscomppars. BUSINESS has seven concepts. Rather than stack two of them on
     the fifth orbit, which would put two ideas at the same radius and quietly
     break the one-concept-one-position rule, the extra orbits are spaced by
     the rule DECLARED IN THE DATA FILE and marked there as not measured.

     This is the treatment Kepler-16 already receives: it has exactly one
     measured planet and four concepts, and orbits 1-3 are spaced by the same
...
/* ── 1a3. A REGION THAT ARRIVES LATER CLAIMS A WORLD ──────────────────
   THIS IS THE LINE THAT MADE A NEW TOPIC NEED A PROGRAMMER.

   Everything else about a region already accommodates one nobody wrote code
   for. The menu takes it, the palette hands it a hue nothing else is using,
   the emblem falls through to a family, the ownership books make it a key.
   Only the star was typed — fourteen regions, fourteen names in MIG_SYSTEM —
   and a region added from the editor could not reach that literal.

   The fourteen above stay typed, and must. Each was CHOSEN: a shape argued
   in the data file to answer what the topic is, and nine assertions name a
   region and a system in the same breath. Nothing here may move them. What
   this adds is what happens to the fifteenth.

   IT TAKES THE NEXT UNCLAIMED SYSTEM FROM THE POOL, in MIGS order. That
   order is the locked corpus's order followed by the order regions were
   declared, and declarations are appended rather than reordered — so the
   same store always produces the same sky, and a region that has claimed a
   world keeps it when the next one arrives.

   IT IS SHOWN WHOLE. A claimed region is new and therefore empty, which is
   the exact case FULL_SYSTEM was written for: without it a region with
   nothing in it is a single light with a name under it rather than a
   planetary system, and there is nothing to fill.

   AND ITS SCALE IS MEASURED OFF THE WORLDS ALREADY IN SERVICE, not chosen.
   WORLD_SCALE runs from 1.1 to 52 because a system's span multiplies it:
   the default 13 would put Kepler-68's 75x span at an outer orbit of 975,
   and Kepler-444's 1.9x at 25, cramming five rings into twelve units. So a
   claimed world is scaled to sit at the same geometric centre — sqrt(inner *
   outer) — as the median world that already has a hand-tuned scale. That
   constant is computed below rather than written down, so it stays true if a
   hand-tuned scale is ever revised, and it cannot be a number somebody liked.

   IF THE POOL IS DRY the region gets no world and renders as the plain star
   it would otherwise have been. That is a shortage to fix by retrieving more
   systems from the archive, exactly as the last six were, and never by
   inventing one. */
var CLAIMED_WORLDS={};       // migId -> {system, scale}, for regions that took one
var WORLD_CENTRE=0;          // the geometric centre the hand-tuned worlds sit at
(function(){
  function spanOf(tpl){
    var a=(tpl&&tpl.semiMajorAxisAU||[]).filter(function(x){ return typeof x==='number'; });
    return a.length>1 ? a[a.length-1]/a[0] : null;
  }
  /* what the set already does. A circumbinary world is left out: LOVE's scale
     of 52 exists so that two stars resolve as two, which is a reason no
     single-star world has. */
  var centres=[];
  Object.keys(WORLD_SCALE).forEach(function(id){
    var tpl=ASTRO[MIG_SYSTEM[id]];
    if(!tpl || tpl.sourceType==='circumbinary-system') return;
    var sp=spanOf(tpl); if(!sp) return;
    centres.push(WORLD_SCALE[id]*Math.sqrt(sp));      // sqrt(inner*outer)
  });
  centres.sort(function(a,b){ return a-b; });
  WORLD_CENTRE=centres.length?centres[Math.floor(centres.length/2)]:ORBIT_R0;

  var spent={}, chosen={};
  Object.keys(MIG_SYSTEM).forEach(function(id){ chosen[id]=1; spent[MIG_SYSTEM[id]]=1; });
  var pool=(ASTRO_DATA.systems||[]).filter(function(sy){
    return sy.reserve && !spent[sy.system]; });
  var standing={}; MIGS.forEach(function(m){ standing[m.id]=1; });

  /* THE INDEX IS SPENT WHETHER THE REGION IS STILL STANDING OR NOT.
     Counting only the visible regions would mean that retiring one moved the
     next one's star, which is the same defect as splicing a retired writing
     out of its list — a region would rearrange the sky by disappearing. So a
     retired region keeps its place in the queue and keeps its world with it. */
  var k=0;
  DECLARED_REGIONS.forEach(function(id){
    if(chosen[id]) return;                       // its system was chosen, not claimed
    if(MIG_CONSTELLATION[id]) return;            // its sky is a constellation
    var sy=pool[k++];
    if(!sy) return;                              // the pool is dry; no world
    if(!standing[id]) return;                    // retired, and holding its index
    var sp=spanOf(sy);
    MIG_SYSTEM[id]=sy.system;
    WORLD_SCALE[id]=sp ? WORLD_CENTRE/Math.sqrt(sp) : ORBIT_R0;
    FULL_SYSTEM[id]=1;
    CLAIMED_WORLDS[id]={ system:sy.system, scale:+WORLD_SCALE[id].toFixed(3),
                         planets:sy.planetCount, span:sp?+sp.toFixed(2):null };
  });
  CLAIMED_WORLDS._spent=k;                       // how much of the queue is gone
  CLAIMED_WORLDS._left=Math.max(0,pool.length-k);
})();
  ...
  if(kon){
    ...
    if(isFull){
      ...
      for(var vk=line.length; vk<slots.length; vk++){
        ...
        var vp={ mig:m.id, slot:vk, r:slots[vk], theta:vth, incl:vin,
          pos:new THREE.Vector3().addVectors(m.pos, localOrbit(slots[vk],vth,vin)) };
        VACANT_PLANETS[m.id].push(vp);
        VACANT_BODIES.push(vp);
      }
...
MIGS.forEach(function(m,i){
  ...
  m.bZ=(0.5-cx)*2.05;
  m.bSide=(i%2===0)?1:-1;
  m.bRad=0.72+((i*5)%3)/3*0.13;
  m.bDepth=0.62+((i*13)%4)/4*0.94;
});
/* TWO NAMES MUST NOT LAND ON THE SAME PIXEL, AND ONLY ONE PLANE DECIDES THAT.

   Alternating hemispheres was supposed to prevent it — "puts neighbours at
   different depths and stops two names from ever landing on the same pixel".
   It does the opposite. Alternating varies X, the default camera is LATERAL,
   and a lateral camera looks straight down X. Two regions on opposite sides at
   the same height and depth-along-Z are exactly in line with the viewer, and
   they collide BECAUSE they were put on opposite sides. PHILOSOPHY at x +18.5
   and BUSINESS at x -64.7 sat 5.3 units apart in the plane the viewer sees,
   out of a brain 156 tall, with their labels drawn on top of each other.
   MY WORKS and SOCIETY, 5.8.

   Two things were mixed into one step and had to be separated:

     - the per-region lateral offset was folded into the direction BEFORE it
       was normalised, and normalising divides all three components by the
       vector's length, so a region given more depth had its y and z shrunk
       more than its neighbours. Depth variety was quietly dragging regions
       around the projected plane. It is now applied along X afterwards, where
       the view cannot see it;

     - the deterministic jitter is large next to a grid cell — up to 0.10 of a
       row that is 0.33 tall — so two adjacent cells could still close up. The
       jitter stays, because it is what keeps the layout from looking like a
       spreadsheet, and instead any pair that comes too close is pushed apart
       IN THE GRID, before the shell mapping, so the mapping still happens once
       and nothing is placed by hand. */
(function spreadInTheViewedPlane(){
  /* a region that never got grid coordinates was skipped above and must stay
     skipped — otherwise it re-enters here with an undefined position */
  var P=MIGS.filter(function(m){ return m.uPos && m.bY!==undefined; }), MIN=30;
  function place(m){
    var dir=new THREE.Vector3(0.32*m.bSide, m.bY, m.bZ).normalize();
    m.bPos=brainShell(dir).multiplyScalar(m.bRad);
    /* depth last, on the axis the lateral view collapses */
    m.bPos.x *= m.bDepth;
  }
  /* Measured on the PLACED positions, not on the grid behind them. The shell
     is not a sphere and normalising is not linear, so two regions pushed apart
     in the grid can be brought back together by the mapping — separating in
     grid space alone fixed PHILOSOPHY and BUSINESS and left MY WORKS and
     SOCIETY 4 units apart. So each round places every region, measures what
     the viewer would actually see, and feeds the correction back into the grid
     coordinates before placing again. 30 units against a median separation of
     145: enough that two names never share a pixel, small enough that the
     arrangement the index chose is what survives. */
  for(var round=0; round<40; round++){
    P.forEach(place);
    var moved=false;
    for(var a=0;a<P.length;a++) for(var b=a+1;b<P.length;b++){
      var p=P[a].bPos, q=P[b].bPos;
      var dy=p.y-q.y, dz=p.z-q.z, d=Math.sqrt(dy*dy+dz*dz);
      if(d>=MIN) continue;
      if(d<1e-6){ dy=1; dz=0; d=1; }            // exactly in line: pick an axis
      var k=(MIN-d)/d*0.006;                    // world units back into grid units
      P[a].bY+=dy*k; P[a].bZ+=dz*k;
      P[b].bY-=dy*k; P[b].bZ-=dz*k;
      moved=true;
    }
    if(!moved) break;
  }
...
var MIG_VISUAL={
  ...
  /* a topic that exists and has not been written into yet. Diffuse,
     uncondensed — not an empty box, but matter that has not become a body. */
  'books'       :{family:'nascent',  branches:0, len:0.00, spread:0.00, rings:0, core:0.30},
  /* things given up. A core with a single ring around it and nothing else:
     what is left after something has been let go of. */
  'wrong'       :{family:'nascent',  branches:0, len:0.00, spread:0.00, rings:1, core:0.26},
  /* BUILDING is hidden and draws nothing, so its species is never asked for.
     Kept because the region still exists in the locked corpus and would want
     its own form again the moment it were unhidden. */
  /* how a thing feels rather than whether it works. A wide diffuse halo around
     a modest core: the region is about the light around an object rather than
     the object, which is also the honest shape of five concepts and three
     beliefs with no system charted under them. */
  'art'         :{family:'halo',     branches:0, len:0.00, spread:0.00, rings:3, core:0.38}
};
...
/* TECHNOLOGY is an M4V red dwarf — about 3200K, the coolest and dimmest star
   in the set, and the reason its planets orbit so close. So this world is red
   where LOVE is amber and LIFE is gold: a deeper, cooler red than either, the
   colour of a star that barely burns. Its bodies are two gas giants and two
...
/* BUSINESS is 55 Cnc, whose signature is 55 Cnc e: a super-Earth on a 17-hour
   orbit, close enough to its star to be molten, and the reason the data calls
   this system "one body almost touching the star, then a huge leap outward".
   So the world takes its colour from that body rather than from the G8V star —
   a hot violet-magenta, which is also the one part of the palette space the
   other six worlds leave open. Money-green was the obvious choice and is a
   cliche about the region rather than a fact about the system. */
var BIZ_VARIANTS={
  a:{ name:'molten violet + ash',
      fog:0xcfc0d2, star:0x8a2f7a, body:0x5b4f6a, orbit:0x6a3a68, accent:0x9c3b86 },
  b:{ name:'deeper magenta + ash',
      fog:0xcbb8d0, star:0x7d2a70, body:0x53475f, orbit:0x613362, accent:0x8f3479 },
  c:{ name:'orchid + slate',
      fog:0xd4c8d8, star:0x97388a, body:0x645874, orbit:0x744276, accent:0xa84a94 }
};
/* SOCIETY is Kepler-11, six planets packed inside an orbit smaller than
   Venus's and lying in almost exactly one plane — the flattest, most layered
   multi-planet system in the set, which is what a region of institutions,
   incentives and tribalism is. Its worlds are the low-density kind, puffy with
   thick hydrogen envelopes rather than rock, so the palette is a pale green
   haze: the one part of the space the other seven leave open, and OBSERVATION's
   verdigris is a cold blue-green against this warmer one. */
var SOC_VARIANTS={
  a:{ name:'pale green haze + moss',
      fog:0xc6d2c0, star:0x4f7a2c, body:0x5d6b4a, orbit:0x486a30, accent:0x639436 },
  b:{ name:'deeper green + olive',
      fog:0xc0cdba, star:0x456d26, body:0x556243, orbit:0x415f2b, accent:0x59872f },
...
var BIZ_PICK=(function(){
  var m=/(^|[#&])bizpal:([abc])/.exec(location.hash||'');
  return m?m[2]:'a';
})();
...
(function(){
  ...
  var tc=TECH_VARIANTS[TECH_PICK]||TECH_VARIANTS.a;
  MIG_PALETTE['technology']={fog:tc.fog, star:tc.star, body:tc.body,
                             orbit:tc.orbit, accent:tc.accent};
  var bz=BIZ_VARIANTS[BIZ_PICK]||BIZ_VARIANTS.a;
  MIG_PALETTE['business']={fog:bz.fog, star:bz.star, body:bz.body,
                           orbit:bz.orbit, accent:bz.accent};
  ...
  /* ART TAKES THE COLOUR THE REMOVED TOPIC LEFT BEHIND.

     ART has eight objects and was the only populated topic without a designed
     palette — it arrived after the twelve were chosen and has been taking
     whatever the automatic search could find between them ever since. Removing
     BUILDING freed a hand-picked colour that this mind has already proved:
     while it was in use every emblem cleared the separation E7 asks for, and
     it is measured against the same eleven neighbours ART sits among.

     So it is inherited rather than invented, and ART stops competing for
     scraps. The BUILDING variants stay where they are: the region still exists
     in the locked corpus and would want its own colour back if it returned. */
  var bd=BUILD_VARIANTS[BUILD_PICK]||BUILD_VARIANTS.a;
  MIG_PALETTE['art']={fog:bd.fog, star:bd.star, body:bd.body,
...
/* A REGION WITHOUT ITS OWN PALETTE STILL NEEDS ITS OWN COLOUR.

   Every uncharted region fell back to the single neutral above, so MY WORKS,
   PSYCHOLOGY and MUSIC were emitting the same blue — measured at an RGB
   distance of 3, which is not a difference anyone can see. The emblems are the
   only things to aim at in the menu, and three of them being one colour is a
   navigation defect rather than a cosmetic one. Adding ART would have made it
   four.

   So the neutral is a BASE, rotated per region. The rotation is derived from
   the region's own index among the uncharted ones, which means it cannot
   collide by construction and a seventeenth region separates itself. */
var NEUTRAL_TINTS={};
(function(){
  /* A REGION WITHOUT ITS OWN PALETTE STILL NEEDS ITS OWN COLOUR.

     Every uncharted region fell back to one neutral blue, so MY WORKS,
     PSYCHOLOGY and MUSIC rendered the same emblem — measured 3 apart in RGB,
     which is not a difference anyone can see. The emblems are the only things
     to aim at in the menu, so three of them being one colour is a navigation
     defect. ART would have made it four.

     Three attempts got this wrong before it worked, and each failure is worth
     keeping: dividing the wheel among the uncharted regions alone ignored
     where the twelve charted palettes sat; spacing by hue landed 13 RGB from
     TECHNOLOGY, because the shader's normalisation makes equal angles unequal
     distances; and rotating the neutral base handed out colours as DARK as
     the base, one of them under the findability floor.

     So: candidates sit on one ring of fixed saturation and lightness, which
     makes every option equally bright, and each region takes the emptiest
     place left on it. Deterministic, no hand placement, and a seventeenth
     region separates itself. */
  function hsl2hex(h, s, l){
    ...
    return ((Math.round((q[0]+m)*255)<<16) | (Math.round((q[1]+m)*255)<<8) |
             Math.round((q[2]+m)*255));
  }
  /* MEASURED IN THE SPACE THE MENU ACTUALLY SHOWS.
     The points shader divides a tint by its own largest channel before drawing
     an emblem, so what reaches the eye is the RATIO between the channels and
     the brightness is thrown away. Two colours of the same hue and different
     lightness arrive identical. Comparing raw palette values therefore left
     PHILOSOPHY and MUSIC 14.9 apart on screen while looking well separated in
     the source. Normalise first, then measure. */
  function norm(hex){
    var r=(hex>>16&255), g=(hex>>8&255), b=(hex&255), mx=Math.max(r,g,b,1);
    return [r/mx*255, g/mx*255, b/mx*255];
  }
  function lumOf(hex){
    var p=norm(hex);
    return (0.2126*p[0] + 0.7152*p[1] + 0.0722*p[2]) / 255;
  }
  /* MEASURED THE WAY THE CHECK MEASURES IT — deltaE in Lab, not RGB.
     Plain RGB distance rates greens far apart that the eye reads as one
     colour: spacing four regions that way put three of them inside 15 deltaE
     of each other while looking well separated in the numbers I was watching.
     Optimising a different metric from the one asserted is how a palette
     passes its own tuning and fails the check. */
  function toLab(c){
    ...
    function g(t){ return t>0.008856 ? Math.pow(t,1/3) : 7.787*t+16/116; }
    var fx=g(X), fy=g(Y), fz=g(Z);
    return [116*fy-16, 500*(fx-fy), 200*(fy-fz)];
  }
  function dist(a, b){
    var p=toLab(norm(a)), q=toLab(norm(b));
    return Math.sqrt((p[0]-q[0])*(p[0]-q[0]) + (p[1]-q[1])*(p[1]-q[1]) +
                     (p[2]-q[2])*(p[2]-q[2]));
  }
  /* Saturation widens the ring in the normalised space the emblems are judged
     in: at 0.56 the sixteen ratios crowded and MUSIC still landed 14.9 from
     PHILOSOPHY, which is under the separation the charted palettes already
     manage between themselves. */
  var RING_S=0.82, RING_L=0.50;
  /* SATURATION IS CHOSEN PER HUE, so the ring is one of constant perceived
     brightness rather than constant saturation.

     Normalisation keeps the ratio between channels and discards the rest, and
     a blue's ratio is simply dark — no amount of lightness rescues it. The
     only way to brighten a blue emblem is to desaturate it. Rejecting the dark
     hues instead left four regions crammed into the warm quarter of the wheel,
     7 apart. Each hue now takes the strongest saturation that still clears the
     brightness target, so the whole wheel stays usable. */
  var LUM_TARGET=0.50;
  function satFor(h, L){
    var best=0.10;
    for(var sat=0.90; sat>=0.10; sat-=0.04){
      if(lumOf(hsl2hex(h, sat, L)) >= LUM_TARGET){ best=sat; break; }
    }
    return best;
  }
  /* scored against `star`, which is the field a MIG node actually draws —
     everything else draws `body`, and spacing those moved nothing in the menu */
  var fixed=[];
  MIGS.forEach(function(m){ if(MIG_PALETTE[m.id]) fixed.push(MIG_PALETTE[m.id].star); });
  var open=MIGS.filter(function(m){ return !MIG_PALETTE[m.id]; });

  /* SEARCHED IN TWO DIMENSIONS, not one. On a single ring the emblems could
     not all be pulled 18 deltaE apart — the charted colours are fixed, and one
     ring left the best arrangement at 16.7. Lightness is the second axis: it
     costs nothing to vary and it is what lets a hue already close to a charted
     one separate by tone instead.

     AND SEARCHED AGAIN AFTERWARDS, WHICH IS THE PART THAT WAS MISSING.

     One pass is greedy: each region in turn takes the seat furthest from
     everything placed so far, which is optimal for that region and can leave
     the LAST one nowhere good. That held while the set of regions did not
     change. Removing a topic changed the number being packed and the order
     they were packed in, and the arrangement collapsed to 13.0 deltaE between
     HUMAN BEHAVIOUR and ART — under the 14 these are held to, which means two
     topics that look alike in the menu.

     The first instinct was to search harder: six times the grid, every hue,
     twelve lightness steps. It came out WORSE, at 12.6, which is the useful
     result — it says the resolution was never the problem. A finer grid only
     lets an early region find a slightly better seat for itself, at the
     expense of whoever sits last.

     So each region is placed, and then every region is asked again, in turn,
     whether it could sit better now that it can see all the others rather than
     only the ones placed before it. That is a few passes of local improvement
     over a set of four, and it converges in three. */
  /* AND HUE IS A CONSTRAINT, NOT ONLY A SCORE.

     dist() divides a colour by its largest channel before measuring, because
     that is what the points shader does. For two colours whose largest channel
     is the SAME one, the surviving ratios can differ enough to score far apart
     while the eye sees one colour twice: HUMAN BEHAVIOUR at #7ff0a6 and ART at
     #0bda6c measured 33 apart internally and rendered 12.8 apart on screen —
     both green, eight degrees of hue between them.

     That is why searching harder never helped. The search was optimising a
     number that had stopped describing the thing being judged. A minimum hue
     separation is crude next to a colour metric and it is the part the metric
     was missing, so it is asserted directly: no emblem may sit within HUE_SEP
     degrees of another, and among the seats that clear it the Lab distance
     still decides. */
  var HUE_SEP=17;
  function hueOf(hex){
    var r=(hex>>16&255)/255, g=(hex>>8&255)/255, b2=(hex&255)/255;
    var mx=Math.max(r,g,b2), mn=Math.min(r,g,b2), d=mx-mn;
    if(d<1e-6) return 0;
    var h;
    if(mx===r) h=60*(((g-b2)/d)%6);
    else if(mx===g) h=60*(((b2-r)/d)+2);
    else h=60*(((r-g)/d)+4);
    return ((h%360)+360)%360;
  }
  function hueGap(a2,b3){ var d=Math.abs(a2-b3)%360; return d>180?360-d:d; }
  function seatFor(avoid){
    var avoidH=avoid.map(hueOf);
    var bh=-1, bgap=-1, bsat=RING_S, bl=RING_L, relaxed=false;
    for(var attempt=0; attempt<2 && bh<0; attempt++){
      var sep=attempt?0:HUE_SEP;
      relaxed=!!attempt;
      for(var L=0.40; L<=0.63; L+=0.025){
        for(var h=0; h<360; h+=2){
          var ok=true;
          for(var q=0;q<avoidH.length;q++) if(hueGap(h,avoidH[q])<sep){ ok=false; break; }
          if(!ok) continue;
          var sat=satFor(h, L), cand=hsl2hex(h, sat, L), gap=1e9;
          for(var k=0;k<avoid.length;k++) gap=Math.min(gap, dist(cand, avoid[k]));
          if(gap>bgap){ bgap=gap; bh=h; bsat=sat; bl=L; }
        }
      }
    }
    if(bh<0){ bh=0; bsat=RING_S; bl=RING_L; }
    return { h:bh, sat:bsat, L:bl, hex:hsl2hex(bh,bsat,bl), gap:bgap, relaxed:relaxed };
  }

  /* pass one: place them, each against everything already placed */
  var seat=[];
  open.forEach(function(m){
    seat.push(seatFor(fixed.concat(seat.map(function(x){ return x.hex; }))));
  });

  /* passes two and after: everyone looks again, now that they can see everyone */
  for(var pass=0; pass<4; pass++){
    var moved=false;
    for(var si=0; si<seat.length; si++){
      var others=fixed.concat(seat.filter(function(_,j){ return j!==si; })
                                  .map(function(x){ return x.hex; }));
      /* THE SEAT IT HAS, SCORED AGAINST THE SAME SET the candidate is scored
         against. The first version compared better.gap with seat[si].gap, and
         those two numbers were measured against DIFFERENT sets — the stored
         gap was computed when the region was first placed, against only the
         regions placed before it. Comparing them meant the pass almost never
         fired, which is why three attempts at this produced 13.0, 12.6 and
         12.8 and looked like the wheel had run out of room. It had not: the
         ring holds 36 colours 14 apart. */
      var cur=1e9;
      for(var oi=0;oi<others.length;oi++) cur=Math.min(cur, dist(seat[si].hex, others[oi]));
      var better=seatFor(others);
      if(better.gap > cur + 0.01){ seat[si]=better; moved=true; }
    }
    if(!moved) break;
  }
  ...
  open.forEach(function(m,i){
    var st=seat[i];
    NEUTRAL_TINTS[m.id]={
      fog:   hsl2hex(st.h, st.sat*0.24, 0.83),
      star:  st.hex,
      body:  hsl2hex(st.h, st.sat*0.49, 0.35),
      orbit: hsl2hex(st.h, st.sat*0.56, 0.37),
      accent:hsl2hex(st.h, st.sat*0.66, 0.45)
    };
  });
})();
function paletteOf(migId){ return MIG_PALETTE[migId] || NEUTRAL_TINTS[migId] || NEUTRAL_PALETTE; }
/* what colour each region was actually given, and whether it designed its own.
   Without this a check can only measure rendered pixels, which cannot say
   whether a clash came from the assignment or from the drawing. */
function paletteReport(){
  var out={};
  MIGS.forEach(function(m){
    var pal=paletteOf(m.id);
    out[m.id]={ own:!!MIG_PALETTE[m.id], star:pal.star };
  });
  return out;
}
/* ── THE WORLD PROFILE ────────────────────────────────────────────────
...
/* The radius of a world's outermost orbit, in scene units — the one it
   ACTUALLY renders, not the last measured one. 55 Cnc has five measured axes
   and seven concepts, so its outermost orbit is a declared extension beyond
   the measurements; reading only the measured axes under-stated the world by
   40% and every range keyed to it fell short, which arrives as a world of
   unnamed lights. Asking orbitalSlots is asking the same function the geometry
   came from. */
...
(function(){
  MIGS.forEach(function(m){
    ...
    var arrive = kon ? (CONST_DATA.derived.meanDistanceLy*CONST_SCALE)
               ...
               /* DERIVED FROM THE WORLD, NOT A CONSTANT PER TYPE.

                  115 was TRAPPIST-1's arrival distance written down as if it
                  were every planetary world's. It is not: GJ 876's outer orbit
                  is 145 scene units against TRAPPIST-1's 70, so it is framed
                  from twice as far and every range keyed to 115 fell short —
                  the labels stopped resolving before the camera arrived. The
                  outer orbit is known here, and 1.65x it reproduces
                  TRAPPIST-1's own 115 exactly, so the approved world is
                  unchanged and the others finally scale. */
               : (type==='planetary') ? outerOrbit(m.id,tpl)*1.65*biasScale
               : 96;
    MIG_WORLD_PROFILE[m.id]={
      ...
      /* Keyed to the world's own ARRIVAL DISTANCE, the same way its
         relationship range already is — a range measured in scene units means
         nothing until you say how far away the camera stands. 1.39x and 0.70x
         reproduce TRAPPIST-1's approved 160 and 80 exactly from its 115. */
      labelStyle: type==='constellation' ? {minor:470, writing:190}
                : {minor:Math.round(arrive*1.39), writing:Math.round(arrive*0.70)},
...
function buildAtlas(){
  ...
  Object.keys(MIG_VISUAL).forEach(function(mid,k){
    ...
    } else if(f==='halo'){                  // ART: the light around the object
      /* A small hard core inside three wide soft rings. Every other species
         draws the thing; this one draws what surrounds it, because the region
         is about how something feels rather than what it is made of. Distinct
         from `nascent` — that has no centre at all, this has a definite one
         and gives most of its area to the atmosphere. */
      core2(R*0.30, 0.30);
      g.lineWidth=R*0.012;
      for(var hr=0; hr<3; hr++){
        var rr=R*(0.48+hr*0.22);
        g.strokeStyle='rgba(255,255,255,'+(0.15-hr*0.04).toFixed(3)+')';
...
if(glOK){
  ...
  var TOTV=placed.length+COMPANIONS.length+CONST_BG.length+VACANT_BODIES.length;
  ...
  placed.forEach(function(n,i){
    ...
    /* AND THE SAME RULE WHEREVER A STAR WOULD EAT ITS OWN INNER BODY.

       LOVE caps its primary so the companion separates instead of being
       swallowed. GJ 876 needs it for the same reason from the opposite cause:
       its innermost planet orbits at 1/16 of the world's span — a real
       property, a 1.94-day orbit at 0.021 AU — so at arrival it lands inside
       the star's own glow, and AI was drawn on top of TECHNOLOGY.

       Judged by the world's own proportions rather than by name: a star is
       capped when its innermost orbit is under an eighth of its outermost.
       TRAPPIST-1 sits at 0.19 and Kepler-33 at 0.27, so neither moves. */
    else if(n.t==='mig'&&templateFor(n.id)){
      var tI=templateFor(n.id), aI=tI.semiMajorAxisAU;
      if(aI && aI.length>1 && (aI[0]/aI[aI.length-1])<0.12) CAP[i]=68;
    }
  ...
  /* ── THE PLANETS NOBODY STANDS ON ────────────────────────────────────
     Written last, after the companions and the background sky, so every
     render-only vertex still sits beyond the last real node and picking
     cannot reach any of them.

     They are drawn as the world's own body form in the world's own body
     colour, because that is what they are: the places this system has. They
     are smaller and dimmer than an occupied one — a place to stand rather than
     something standing — and the moment a note takes one it stops being in
     this list and is drawn as a real object instead. REG is set so the world
     highlight reaches them with everything else in their world; without it
     hovering MUSIC in the menu would light the star and leave its planets
     behind. */
  VACANT_BODIES.forEach(function(v,vi){
    var i=placed.length+COMPANIONS.length+CONST_BG.length+vi;
    P[i*3]=v.pos.x; P[i*3+1]=v.pos.y; P[i*3+2]=v.pos.z;
    var order=Object.keys(MIG_VISUAL).indexOf(v.mig);
    var gi=order>=0?GLYPHS.length+order:0;      // the world's own stellar form
    CELLA[i*2]=gi%ATLAS; CELLA[i*2+1]=Math.floor(gi/ATLAS);
    /* between a writing (44) and a concept (62), and under the cap every other
       body obeys, so an empty planet can never out-shout an occupied one */
    SZ[i]=52;
    CAP[i]=CAP_DEFAULT;
    var pal=paletteOf(v.mig);
    var cv=new THREE.Color(pal.body);
    COL[i*3]=cv.r; COL[i*3+1]=cv.g; COL[i*3+2]=cv.b;
    EMPH[i]=0.52;                                // dimmer than the least-connected object
    REG[i]=(migIndex[v.mig]===undefined?-1:migIndex[v.mig]);
  });
  ...
  Object.keys(ORBITS).forEach(function(mid){
    ...
    /* THE PLANET ITSELF IS NOT DRAWN HERE ANY MORE. It was a small circle in
       this same line buffer, which is a shader written for paths: thin, faint,
       and faded hard with distance. At the range a world is framed from, that
       read as a smudge on a ring rather than as a planet — the orbits were
       visibly there and looked empty.

       An empty planet is now a real body in the sprite cloud, written with the
       companions and the background sky, so it is lit, sized and coloured by
       the same renderer as everything else in the world. See VACANT_BODIES. */
  });
...
var elWhere=document.getElementById('where'), elGloss=document.getElementById('gloss'),
    ...
    readSections=document.getElementById('readSections');
...
function row(n, meta, onClick, current){
  ...
  b.appendChild(l);
  /* A CHARTED SYSTEM IS NOT NAMED TWICE; AN UNCHARTED ONE IS STILL DECLARED.
     Every MIG row used to carry its planetary system beneath the region's own
     name — TRAPPIST-1 under PHILOSOPHY — while the sky already writes that
     same name beside the world itself. Saying it twice made the menu look like
     it held two facts where it held one, so the charted ones were removed.

     That took something else with it. The sky names only the worlds that HAVE
     a system, so the twelve regions without one went from declaring "not yet
     charted" to saying nothing at all — and silence is not a declaration. A
     region that quietly shows no heritage is indistinguishable from one whose
     label failed to render, which is exactly the ambiguity the line existed to
     remove. So the declaration comes back for those, and only those: the menu
     now carries what the sky cannot say. */
  if(n.t==='mig'){
    var sLabel=sourceLabelOf(n.id);
    if(sLabel==='not yet charted'){
      var src=document.createElement('span');
      src.className='src'; src.setAttribute('data-src',n.id);
      src.textContent=sLabel;
      b.appendChild(src);
      b.setAttribute('aria-label',(n.label||n.id)+' — '+sLabel);
    }
  }
...
function paintDOM(){
  ...
  if(n.src) put(group('This writing',[ row(n,'Read it',
    function(){ openReader(n.id); }) ]));
  /* THE EDITOR HAD NO WAY IN HERE AT ALL. A region's page gets paintRegion and
     every object in it can be edited or retired from the row it stands on;
     this page — the one that lists what a thing CONNECTS to — had no hook, so
     a relationship could be added from the form and never taken away from the
     place it is actually wrong. Same shape as paintRegion: the rows are
     already painted, and the controls are appended to them. */
  if(window.__editor && window.__editor.paintConnections)
    window.__editor.paintConnections(n.id, elGroups);
...
function openReader(id){
  ...
  /* THE SECTIONS UNDER THE STATEMENT.
     ...
     A writing used to be exactly one sentence and the reader drew exactly one.
     It may now carry sections after it — a quote and then what the quote
     meant, and however many more are added later — so they are painted here,
     in order, and the container is emptied first because the reader is one
     element reused for every writing rather than one per note.

     Heading optional, body required: a section with nothing in its body is not
  ...
  (n.sections||[]).forEach(function(sec){
    ...
    var wrap=document.createElement('div'); wrap.className='docsec';
    if(String(sec.heading||'').trim()){
      var h=document.createElement('p'); h.className='docsec-h';
      h.textContent=String(sec.heading).trim();
      wrap.appendChild(h);
    }
...
window.__v02={
  ...
  model:function(){
    return {
      ...
      /* vacant and edited are what the editor reads to decide what it may
         offer: a vacancy can be claimed but not opened, and an edited writing
         shows its correction rather than the corpus text. */
      nodes: NODES.map(function(n){
        return {id:n.id, label:n.label, t:n.t, mig:n.mig,
                line:n.line||null, register:n.register||null, src:n.src||null,
                crosses:(n.crosses||[]).slice(),
                /* WITHOUT THIS THE EDITOR COULD ADD SECTIONS AND NEVER SEE THE
                   ONES ALREADY THERE, so opening a writing to add a third
                   section would have silently replaced the first two. */
                sections:(n.sections||[]).map(function(sc){
                  return {heading:sc.heading||'', body:sc.body||''}; }),
                vacant:!!n.vacant, retiredAt:n.retiredAt||null, edited:!!n.edited};
      }),
  ...
  graph:function(){ return {nodes:NODES.length, links:LINKS.length,
    /* the count the brain is supposed to draw: relationships whose two ends
       live in different regions. Exposed so a check can compare the DRAWING
  ...
  overlay:function(){
    ...
    /* FOUND BY WHAT IT IS, NOT BY THE ID A WORKAROUND GAVE IT. This looked up
       'psychology-behaviour', the collision-safe key the concept was moved to
       when PSYCHOLOGY was promoted to a region. That promotion is undone and
       the rename with it, so the lookup found nothing and a check reported the
       concept missing when it had simply gone back to its own name. */
    var p=null;
    for(var pi=0;pi<MINORS.length;pi++)
      if(MINORS[pi].label==='PSYCHOLOGY'){ p=byId[MINORS[pi].id]; break; }
  ...
  palettes:function(){ return paletteReport(); },
  species:function(){
    var out={};
    MIGS.forEach(function(m){
  ...
  /* WHERE THE SOURCE CLAIM NOW LIVES. It used to be declared under every menu
     row, including "not yet charted" beneath the regions that have no system.
     The menu no longer repeats what the sky already says, so the audit reads
     the sky: a charted world writes its system beside itself, and a world with
     no system writes nothing. What is testable therefore changed shape - it is
     no longer "every region states a source", it is "no region states one it
     does not have, and every one that has a source states it". */
  skyLabels:function(){
    return Object.keys(labelEls).filter(function(id){
      return byId[id] && byId[id].t==='mig';
    }).map(function(id){
  ...
  /* WHICH REGIONS TOOK A WORLD RATHER THAN BEING GIVEN ONE.
  ...
  claims:function(){
    return { centre:+WORLD_CENTRE.toFixed(3),
             ...
             pool:(ASTRO_DATA.systems||[]).filter(function(s){ return s.reserve; })
                   .map(function(s){ return s.system; }),
             assigned:Object.keys(MIG_SYSTEM).map(function(k){ return MIG_SYSTEM[k]; }) };
  },
  systems:function(){
    var out={};
    MIGS.forEach(function(m){
  ...
  constellation:function(mid){
    ...
    return {
      ...
      /* the empty planets of a declared world are render-only too */
      vacantBodies:VACANT_BODIES.length,
  ...
  binary:function(){
    ...
    out.renderOnly=COMPANIONS.length+CONST_BG.length+VACANT_BODIES.length;  // every body that is not an idea
  ...
  astro:function(){
    ...
    Object.keys(MIG_SYSTEM).forEach(function(mid){
      ...
      /* periods as well as axes, because GJ 876's identity is a RESONANCE —
         a relationship between orbital periods — and a check cannot verify
         that the rendered radii encode it without the periods to derive it
         from. */
      out.assigned[mid]=tpl?{system:tpl.system, planets:tpl.planetCount,
        axes:tpl.semiMajorAxisAU, periods:tpl.orbitalPeriodDays||null,
        planetCountDeclared:tpl.planetCount||null,
        /* exposed so a check can prove that geometry beyond the measurements
           is DECLARED as such and never presented as a measurement */
        illustrative:tpl.illustrative||null,
        confidence:tpl.confidence, sourceType:tpl.sourceType}:null;
    });
    Object.keys(ORBITS).forEach(function(mid){
