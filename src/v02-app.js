...
var MIG_SYSTEM={ 'philosophy':'TRAPPIST-1', 'love':'Kepler-16', 'movies':'HR 8799',
                 ...
                 /* the last unassigned system, and the one the archive itself
                    nominated for a topic with almost nothing in it */
                 'wrong':'Proxima Cen' };
...
(function(){
  ...
  var taken=[];
  MIGS.forEach(function(m){ if(MIG_PALETTE[m.id]) taken.push(MIG_PALETTE[m.id].star); });
  MIGS.filter(function(m){ return !MIG_PALETTE[m.id]; }).forEach(function(m){
    /* SEARCHED IN TWO DIMENSIONS, not one. On a single ring the sixteen
       emblems could not all be pulled 18 deltaE apart — the twelve charted
       colours are fixed, and one ring left the best arrangement at 16.7.
       Lightness is the second axis: it costs nothing to vary and it is what
       lets a hue already close to a charted one separate by tone instead. */
    var best=-1, bestGap=-1, bestSat=RING_S, bestL=RING_L;
    for(var L=0.40; L<=0.63; L+=0.035){
      for(var h=0; h<360; h+=3){
        var sat=satFor(h, L), cand=hsl2hex(h, sat, L), gap=1e9;
        for(var k=0;k<taken.length;k++) gap=Math.min(gap, dist(cand, taken[k]));
        if(gap>bestGap){ bestGap=gap; best=h; bestSat=sat; bestL=L; }
      }
    }
    if(best < 0){ best=0; bestSat=RING_S; bestL=RING_L; }
    var star=hsl2hex(best, bestSat, bestL);
    taken.push(star);
    NEUTRAL_TINTS[m.id]={
      fog:   hsl2hex(best, bestSat*0.24, 0.83),
      star:  star,
      body:  hsl2hex(best, bestSat*0.49, 0.35),
      orbit: hsl2hex(best, bestSat*0.56, 0.37),
      accent:hsl2hex(best, bestSat*0.66, 0.45)
    };
  });
})();
